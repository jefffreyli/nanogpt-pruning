...
class GPTConfig:
    ...
    # layers (0-based) where you do token selection
...
class GPT(nn.Module):
    ...
    def get_pruning_stats(self, idx: torch.Tensor):
        """
        Run a forward pass and report how many tokens are kept at each layer.

        Args:
            idx: Input token indices of shape (B, T)

        Returns:
            List of dictionaries with pruning statistics per reduction layer
        """
        if not self.use_token_reduction:
            return []
        ...
        stats = []
        cumulative_keep_ratio = 1.0

        for layer_idx, block in enumerate(self.transformer.h):
            prev_x = x
            x_new = block(prev_x)

            if str(layer_idx) in self.policies:
                policy = self.policies[str(layer_idx)]
                probs = policy(prev_x)  # (B, T), p(select)
                ...
                # Calculate keep ratio at this layer
                num_kept = alive.float().sum(dim=1).mean().item()
                keep_ratio = num_kept / t
                cumulative_keep_ratio = keep_ratio
                ...
                x = x_new
                # For non-reduction layers, keep ratio stays the same as previous
                stats.append({
                    "layer": layer_idx,
                    "avg_tokens_kept": t * cumulative_keep_ratio,
                    "keep_ratio": cumulative_keep_ratio,
                    "is_reduction_layer": False,
                })
